        ...
        <table className="w-full border-collapse border border-gray-400 text-xl">
          <thead>
            <tr className="bg-gray-100 border-b border-gray-400">
              {["visitor_name", "time_slot", "date", "day_of_week", "seat_number"].map((key) => (
                <th
                  key={key}
                  className={`border px-4 py-2 cursor-pointer ${sortKey === key ? "bg-blue-200" : ""}`}
                  onClick={() => handleSort(key as keyof Reservation)}
                >
                  {key === "visitor_name" && "予約者名"}
                  {key === "time_slot" && "予約枠"}
                  {key === "date" && "年月日"}
                  {key === "day_of_week" && "曜日"}
                  {key === "seat_number" && "座席番号"}
